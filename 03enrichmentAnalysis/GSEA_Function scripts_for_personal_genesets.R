...
GSEA <- function(
  ...
  for (nk in 1:n.tot) {
    ...
    O <- GSEA.GeneRanking(A, class.labels, gene.labels, call.nperm, permutation.type = perm.type, sigma.correction = "GeneCluster", fraction=fraction, replace=replace, reverse.sign = reverse.sign)   ########Modified,O should be the results but not function######
    gc()
  ...
  names(report) <- c("GS", "SIZE", "SOURCE", "ES", "NES", "NOM p-val", "FDR q-val", "FWER p-val", "Tag /%", "Gene /%", "Signal", "FDR (median)", "glob.p.val")   ######Transfomr \ to /########
  #       print(report)
  report2 <- report
 ...
 if (output.directory != "")  {
  ...
  delta.string <- paste("Corr. Area Bias to area.phen", abs(area.bias), "/", sep="", collapse="")    ######################## transform \ to / ########
  ...
  leg.txt <- paste("", phen1, "", sep="", collapse="")    ####################### transform "/""to ""/"########
  text(x=1, y=-0.05*max.corr, adj = c(0, 1), labels=leg.txt, cex = 0.9)
  
  leg.txt <- paste("", phen2, "", sep="", collapse="")    ####################### transform "/"" to""/"########
...
GSEA.Analyze.Sets <- function(
  ...
  files <- file.list[regexpr(pattern = "/.report/.", file.list) > 1]   ################################################ note  transform \ to /##############
  max.sets <- length(files)
  
  set.table <- matrix(nrow = max.sets, ncol = 5)
  
  for (i in 1:max.sets) { 
    temp1 <-  strsplit(files[i], split="/.report/.")  ################################################ note  transform \ to /##############
    temp2 <-  strsplit(temp1[[1]][1], split="//.")  ################################################ note  transform \ to /##############
    s <- length(temp2[[1]])
    prefix.name <- paste(temp2[[1]][1:(s-1)], sep="", collapse="")
    set.name <- temp2[[1]][s]
    temp3 <-  strsplit(temp1[[1]][2], split="//.")   ################################################ note  transform \ to /##############
    phenotype <- temp3[[1]][1]
    seq.number <- temp3[[1]][2]
    dataset <- paste(temp2[[1]][1:(s-1)], sep="", collapse="/.")  ################################################ note  transform \ to /##############
